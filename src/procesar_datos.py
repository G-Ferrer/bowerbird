...
def menu_procesado_datos():
    ...
    print " 2. Bottom 10 cartas"
    print " 3. Exportar matriz de adyacencia (.csv)"
    print " 4. Mejor baraja (fuerza bruta matriz de adyacencia .csv. RECOMENDADO)"
    print " 5. Mejores barajas (fuerza bruta)"
    print " 6. Peores barajas (fuerza bruta)"
    print " 7. Exportar matriz de adyacencia (.npy)"
    print " 8. Lista de cartas"
    print "\n 0. Salir"
...
def elegir_cantidad_cpus_mejores():
    ...
    while int_valido == False:
        ...
        os.system('clear')
        print ' MEJORES BARAJAS (FUERZA BRUTA)\n'

    return cpus


def elegir_cantidad_cpus_peores():
    ...
    while int_valido == False:
        ...
        os.system('clear')
        print ' PEORES BARAJAS (FUERZA BRUTA)\n'
...
def bottom_10():
    ...
    # Calcula las 10 cartas menos usadas.
    # Si varias cartas se usan la misma cantidad de veces, las mete dentro de la misma posición.
    ...
    # Para poder reutilizar el código de top_10(), le resto al mayor valor, el valor de cada carta.
    # Las veces que aparece cada carta es la cantidad mayor menos la cantidad guardada.
    
    num_mayor = lista_conteo_ordenado[0][1]

    for i in range(len(lista_conteo_ordenado)):
        lista_conteo_ordenado[i][1] = num_mayor - lista_conteo_ordenado[i][1]

    # Reordeno.
    lista_conteo_ordenado.sort(key=lambda x: x[1])
    lista_conteo_ordenado = list(reversed(lista_conteo_ordenado))

    # Imprimo las 10 cartas menos usadas. En el caso de que algunas de las cartas se usen la misma cantidad de veces,
    # las muestro dentro del mismo puesto

    print '\n'
    print ' BOTTOM 10 CARTAS'
    print '\n'
    print ' %s >> %s  (%s %%)' % (lista_conteo_ordenado[0][0], num_mayor - lista_conteo_ordenado[0][1], round(((num_mayor - lista_conteo_ordenado[0][1])/num_total_barajas)*100,1))
    cuenta_top_10_cartas = 1
    ...
    for i in range(1, len(lista_conteo_ordenado)):
        ...
        if lista_conteo_ordenado[i][1] < lista_conteo_ordenado[i-1][1] and cuenta_top_10_cartas < 10:
            print '\n'
            print ' %s >> %s  (%s %%)' % (lista_conteo_ordenado[i][0], num_mayor - lista_conteo_ordenado[i][1], round(((num_mayor - lista_conteo_ordenado[i][1])/num_total_barajas)*100,1))
            cuenta_top_10_cartas = cuenta_top_10_cartas + 1
...
def bottom_10_a_archivo(ruta_guardar_archivo):
    ...
    with open(ruta_guardar_archivo, "w+") as archivo:
        ...
        archivo.write('\n')
        archivo.write(' BOTTOM 10 CARTAS')
        archivo.write('\n\n')
        archivo.write(' %s >> %s  (%s %%)' % (lista_conteo_ordenado[0][0], num_mayor - lista_conteo_ordenado[0][1], round(((num_mayor - lista_conteo_ordenado[0][1])/num_total_barajas)*100,1)))
        cuenta_top_10_cartas = 1
        ...
        for i in range(1, len(lista_conteo_ordenado)):
            ...
            if lista_conteo_ordenado[i][1] < lista_conteo_ordenado[i-1][1] and cuenta_top_10_cartas < 10:
                archivo.write('\n\n')
                archivo.write(' %s >> %s  (%s %%)' % (lista_conteo_ordenado[i][0], num_mayor - lista_conteo_ordenado[i][1], round(((num_mayor - lista_conteo_ordenado[i][1])/num_total_barajas)*100,1)))
                cuenta_top_10_cartas = cuenta_top_10_cartas + 1
...
def mejor_baraja_fuerza_bruta_thread_C(comb_inicial, comb_final, matriz_relaciones_simetrica, num_thread, ruta_carpeta_resultados):
    ...
    # Ruta ejecutable
    ruta_ejecutable = ruta_principal + '/src/fuerza_bruta_thread_top'
    ...
    nombre_archivo_resultados = ruta_carpeta_resultados + '/resultado_thread_' + num_thread + '.txt'

    with open(nombre_archivo_resultados, "w+") as output:
        call(llamar_programa_c, stdout=output)


def peor_baraja_fuerza_bruta_thread_C(comb_inicial, comb_final, matriz_relaciones_simetrica, num_thread, ruta_carpeta_resultados):

    llamar_programa_c = []

    # Ruta ejecutable
    ruta_ejecutable = ruta_principal + '/src/fuerza_bruta_thread_bottom'
    llamar_programa_c.append(ruta_ejecutable)
...
def mejor_baraja_fuerza_bruta_main():
    ...
    cpus = elegir_cantidad_cpus_mejores()
    ...
    bottom_10_a_archivo(ruta_final + '/bottom_10_' + hora_inicio + '.txt')
...
def peor_baraja_fuerza_bruta_main():
    ...
    # Número de CPU's que voy a usar
    cpus = elegir_cantidad_cpus_peores()
    ...
    if cpus >= len(cartas):
        pool = Pool(processes=len(cartas))
        for i in range(len(lista_comb_repartidas)):
            pool.apply_async(peor_baraja_fuerza_bruta_thread_C, [lista_comb_repartidas[i][0], lista_comb_repartidas[i][1], matriz_relaciones_simetrica, str(i), ruta_carpeta_resultados])
    else:
        pool = Pool(processes=cpus)
        for i in range(cpus):
            pool.apply_async(peor_baraja_fuerza_bruta_thread_C, [lista_comb_repartidas[i][0], lista_comb_repartidas[i][1], matriz_relaciones_simetrica, str(i), ruta_carpeta_resultados])
...
def menu_peor_baraja_fuerza_bruta():

    peor_baraja_fuerza_bruta_main()
...
def exportar_matriz_adyacencia_csv():

    # Permite guardar la matriz de relaciones (que realmente se trata de una matriz de adyacencia) como un archivo ".csv".
    ...
    hora = time.strftime("%d-%m-%y") + '_' + time.strftime("%H:%M:%S")

    with open(ruta_principal + '/files/resultados_procesado_datos/' + 'matriz_adyacencia_' + hora + '.csv', "wb") as f:
    	writer = csv.writer(f)
    	writer.writerows(matriz_relaciones_simetrica)

    print " Datos de la matriz de adyacencia guardados en: %s\n" % ruta_principal + '/files/resultados_procesado_datos/' + 'matriz_adyacencia_' + hora + '.csv'

    print "\n"


def menu_exportar_matriz_adyacencia_csv():

    exportar_matriz_adyacencia_csv()

    raw_input(" Pulse enter para volver...")
    menu_ejecucion_procesado('9')


def exportar_matriz_adyacencia_npy():

    # Permite guardar la matriz de relaciones (que realmente se trata de una matriz de adyacencia) como un archivo ".npy". Estos archivos son los que utiliza numpy para exportar/importar datos. De esta manera, más adelante se podrán realizar operaciones matemáticas con numpy sobre esta matriz de adyacencia.
    ...
    numpy.save(ruta_principal + '/files/resultados_procesado_datos/' + 'matriz_adyacencia_' + hora, matriz_relaciones_simetrica)

    print " Datos de la matriz de adyacencia guardados en: %s\n" % ruta_principal + '/files/resultados_procesado_datos/' + 'matriz_adyacencia_' + hora + '.npy'

    print "\n"


def menu_exportar_matriz_adyacencia_npy():

    exportar_matriz_adyacencia_npy()

    raw_input(" Pulse enter para volver...")
    menu_ejecucion_procesado('9')


def menu_mejor_baraja_fuerza_bruta_matriz_adyacencia_csv():

    print " NOTA: por defecto se están usando 8 cpu's.\n"
    print " Modificar la constante NUMERO_THREADS_A_USAR del archivo fuerza_bruta_matriz_adyacencia_csv_paralelo.c"
    print " y recompilar si se desea modificar el número de cpu's utilizadas.\n"
    print " Más información sobre cómo compilar en el propio archivo.\n"

    llamar_programa_c = []

    # Ruta ejecutable
    ruta_ejecutable = ruta_principal + '/src/fuerza_bruta_matriz_adyacencia_csv_paralelo'
    llamar_programa_c.append(ruta_ejecutable)

    # Ruta archivo .csv
    Tk().withdraw()
    ruta_archivo_csv = askopenfilename(filetypes = ((".csv files", "*.csv"), ("all files", "*.*")))
    llamar_programa_c.append(ruta_archivo_csv)

    hora_inicio = time.strftime("%d-%m-%y") + '_' + time.strftime("%H:%M:%S")
    ruta_carpeta_resultados = ruta_principal + '/files/resultados_procesado_datos/' + time.strftime("%d-%m-%y") + '_' + time.strftime("%H:%M:%S")
    os.makedirs(ruta_carpeta_resultados)

    nombre_archivo_resultados = ruta_carpeta_resultados + '/resultado_mejor_baraja_matriz_adyacencia.txt'
...
acciones_menu_procesado = {
    '1': menu_top_10,
    '2': menu_bottom_10,
    '3': menu_exportar_matriz_adyacencia_csv,
    '4': menu_mejor_baraja_fuerza_bruta_matriz_adyacencia_csv,
    '5': menu_mejor_baraja_fuerza_bruta,
    '6': menu_peor_baraja_fuerza_bruta,
    '7': menu_exportar_matriz_adyacencia_npy,
    '8': imprimir_lista_cartas,
