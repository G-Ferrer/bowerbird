...
// NOTA: este programa buscaba sustituir el anterior programa 'fuerza_bruta_threat_top'.
// Finalmente se decide no hacerlo. Se deja el presente programa de manera anecdótica (legacy).

// gcc -o <nombrePrograma> <nombrePrograma.c>
// Llamar como ./<nombre_ejecutable> <archivo.csv>
...
#define NUMERO_THREADS_A_USAR ( 2 <= omp_get_max_threads() ? 2 : omp_get_max_threads() )  // "Operador Ternario"
...
MejorGrupo encontrarElMejorGrupo(uint64_t posicionPrimerGrupo, uint64_t posicionUltimoGrupo, size_t numeroDeFilas, size_t numeroDeColumnas, uint64_t matrizAdyacencia[numeroDeFilas][numeroDeColumnas])
{
    ...
    while (okay && posicionPrimerGrupo < posicionUltimoGrupo)
    {
		...
		// mejor grupo hasta este momento. Si es así, se guarda y se
		// imprime por pantalla el nuevo mejor grupo
...
// número de filas
    ...
    encontrarElMejorGrupo(posicionPrimerGrupo, posicionUltimoGrupo, numeroDeFilas, numeroDeColumnas, matrizAdyacencia);
